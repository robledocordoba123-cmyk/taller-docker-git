...
def obtener_datos():
    ...
    if datos_cache:
        return jsonify({"origen": "cache", "datos": eval(datos_cache), "ttl_restante": redis_client.ttl(cache_key)})

    datos = obtener_datos_lentos()
    redis_client.setex(cache_key, 60, str(datos))
    return jsonify({"origen": "base_de_datos", "datos": datos, "cacheado_por": "60 segundos"})
